...
const ConfigPanel: FC<ConfigProps> = ({
  config,
  models
}) => {
  ...
  } = useInput(config.configs[ProviderType.GPT3]?.apiKey ?? '');
  const model = useRef(config.configs[ProviderType.GPT3]?.model ?? models[0]);
  const {
    setToast
  } = useToasts();
  const save = useCallback(async () => {
    if (tab === ProviderType.GPT3) {
      ...
      if (!model.current.value || !models.includes(model.current.value)) {
  ...
  }, [apiKeyBindings.value, model.current.value, models, setToast, tab]);
  return <div className="flex flex-col gap-3">
        ...
        <Tabs.Item label="OpenAI API" value={ProviderType.GPT3}>
          <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-row gap-2">
              <Select scale={2 / 3} ref={model} placeholder="model">
                {models.map(m => <Select.Option key={m} value={m}>
                    {m}
                  </Select.Option>)}
              </Select>
      ...
      <Button scale={2 / 3} ghost style={{
      width: 20
    }} type="success" onClick={save}>
        保存
      </Button>
    </div>;
};
